...
public class FinanceResultRepository {
    ...
    private FinanceResult financeResultMapper(ResultSet rs, int rowNumber) {
        try {
            FinanceResult financeResult = new FinanceResult();
            financeResult.setIncome(rs.getBigDecimal("income"));
            financeResult.setOutcome(rs.getBigDecimal("outcome"));
